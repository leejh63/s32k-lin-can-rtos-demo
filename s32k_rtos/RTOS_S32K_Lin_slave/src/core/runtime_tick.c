// LPTMR 기반 ISR/service runtime tick 구현 파일이다.
// LIN slave task 문맥 공용 시간 기준은 FreeRTOS tick이 맡고,
// 이 계층은 필요한 경우에만 별도 service용 base interrupt를 제공한다.
...
// task 문맥 공용 시간축은 이 단계에서 FreeRTOS tick으로 분리했다.
...
// ISR/service 경로가 공유할 때만 millisecond 누적값을 읽어 온다.
