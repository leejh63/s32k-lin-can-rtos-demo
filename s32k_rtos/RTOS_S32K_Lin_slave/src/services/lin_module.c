...
// 주의: 현재는 마지막 PID 한 슬롯만 저장한다.
// 지금 구조에는 충분하지만, 이벤트 밀도가 높아지면 queue형 저장으로 바꾸는 편이 안전하다.
...
// status PID에는 현재 cache를 송신하고, ok PID에는 token 수신으로 진입한다.
...
// 주의: callback에서는 event 적재만 하고, 실제 상태 전개는 여기서 한다.
// 이 함수가 길어지더라도 IRQ 문맥을 가볍게 유지하는 방향은 계속 지키는 편이 낫다.
void LinModule_TaskFast(LinModule *module, uint32_t now_ms)
{
    ...
    // 1) callback이 남긴 event bit와 마지막 PID를 한 번에 가져온다.
    ...
    // 3) PID 인식 -> RX 완료 -> TX 완료 순으로 후속 단계를 이어 간다.
...
// 주의: 현재 구조는 poll 슬롯 하나에서 status와 ok 전송이 번갈아 쓰인다.
// 단순하고 안정적이지만 throughput이나 반응성 면에서는 보수적인 설계다.
void LinModule_TaskPoll(LinModule *module, uint32_t now_ms)
{
    ...
    // 승인 token이 대기 중이면 이번 슬롯은 ok PID에 우선 사용한다.
...
// 주의: 현재는 publish할 때마다 fresh 비트를 1로 세팅한다.
// slave 프로젝트에서는 단순하고 직관적이지만, 이후 "진짜 새 샘플" 구분이 필요하면 별도 정책이 필요하다.
