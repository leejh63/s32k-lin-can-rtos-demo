...
// RGB LED PWM 채널과 active polarity 정보를 공용 LedConfig 형태로 묶어 반환한다.
...
// 확장 시에는 입출력 성격별로 파일을 더 나누는 것도 적절하다.
