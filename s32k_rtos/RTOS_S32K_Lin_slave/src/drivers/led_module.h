...
typedef enum
{
    ...
    LED_PATTERN_CUSTOM_SOLID,
    LED_PATTERN_RED_BLINK,
    LED_PATTERN_GREEN_BLINK
} LedPattern;

// LED 모듈에 필요한 PWM 출력 정보 구조체다.
// Runtime IO가 이 값을 채워 넣어,
// LED 로직이 concrete FTM channel 정의와 분리된 의미 기반 코드로 남게 한다.
typedef struct
{
    uint8_t  red_channel_index;
    uint8_t  green_channel_index;
...
typedef struct
{
    ...
    uint16_t   solid_red_duty;
    uint16_t   solid_green_duty;
} LedModule;
