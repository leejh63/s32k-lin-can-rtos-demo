...
// slave2 LED gradient가 green -> yellow로 넘어가는 ADC 기준점을 반환한다.
uint16_t RuntimeIo_GetSlave2AdcSafeMax(void)
{
    return RUNTIME_IO_ADC_SAFE_MAX;
}

// slave2 LED gradient가 yellow -> red로 끝나는 ADC 기준점을 반환한다.
uint16_t RuntimeIo_GetSlave2AdcEmergencyMin(void)
{
    return RUNTIME_IO_ADC_EMERGENCY_MIN;
}
...
// 센서 종류가 더 늘어나면 역할별 설정 묶음을 조금 더 구조화해 두는 편이 가독성이 높다.
