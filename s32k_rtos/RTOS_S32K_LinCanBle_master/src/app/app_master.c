                                    ...
                                    const char *result_text)
{
    ...
    if (result_text != NULL)
    {
        AppCore_QueueLinResultText(app, result_text);
    }
}
                                                                 ...
                                                                 const char **out_result_text)
{
    ...
    if (status->fault != 0U)
    {
        ...
        {
            AppCore_QueueLinResultText(app, "[cancel] lin fault");
        }
    ...
    if (status->zone == LIN_ZONE_EMERGENCY)
    {
        ...
        {
            AppCore_QueueLinResultText(app, "[cancel] ok relay aborted by emergency");
        }
        return APP_MASTER_RELAY_RESULT_CANCELLED;
    }
...
void AppMaster_RequestOk(AppCore *app, uint32_t now_ms)
{
    ...
    {
        AppCore_QueueLinResultText(app, "[wait] lin not ready");
        return;
    }
    ...
    if (AppMaster_IsOkRelayActive(app) != 0U)
    {
        ...
        AppCore_QueueLinResultText(app, "[busy] ok already pending");
        return;
    }
    ...
    if (status_ready == INFRA_STATUS_EMPTY)
    {
        ...
        AppCore_QueueLinResultText(app, "[wait] no lin status");
        return;
    }
    ...
    if (status_ready != INFRA_STATUS_OK)
    {
        ...
        AppCore_QueueLinResultText(app, "[wait] lin status stale");
        return;
    }
    ...
    if (status.fault != 0U)
    {
        ...
        AppCore_SetLinLinkText(app, "fault");
        AppCore_QueueLinResultText(app, "[deny] lin sensor fault");
        return;
    }
    ...
    if (status.zone == LIN_ZONE_EMERGENCY)
    {
        ...
        AppCore_QueueLinResultText(app, "[deny] adc still emergency");
        return;
    }
    ...
    if (status.emergency_latched == 0U)
    {
        ...
        AppCore_QueueLinResultText(app, "[info] slave not locked");
        return;
    }
    ...
    if (request_status != INFRA_STATUS_OK)
    {
        ...
        AppCore_QueueLinResultText(app, "[busy] lin ok tx busy");
        return;
    }
    ...
    AppCore_QueueLinResultText(app, "[queued] slave1 ok -> lin ok");
}
...
// 현재 규모에서는 가독성을 유지할 수 있지만, 추후에는 policy/presentation 분리 후보로 검토하는 것이 적절하다.
                                ...
                                uint8_t *out_response_code)
{
    ...
    {
        AppCore_QueueCanPresentation(app, APP_CORE_CAN_PRESENT_CAN_INPUT_TEXT, "button in");
        AppCore_QueueMasterOkRequestFromCan(app);
...
void AppMaster_AfterLinPoll(AppCore *app, uint32_t now_ms)
{
    ...
    if (status_ready == INFRA_STATUS_OK)
    {
        ...
        if (relay_result != APP_MASTER_RELAY_RESULT_NONE)
        {
            ...
            {
                AppCore_QueueLinResultText(app, relay_result_text);
            }
            return;
        }
    }
    ...
    if (request_status == INFRA_STATUS_OK)
    {
        ...
        app->ok_relay.last_retry_ms = now_ms;
        AppCore_SetLinLinkText(app, "ok pending");
        AppCore_QueueLinResultText(app, "[retry] lin ok request");
    }
}
