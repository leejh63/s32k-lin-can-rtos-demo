...
// FreeRTOS 아래에서는 CAN IRQ 우선순위를 명시해 두는 편이 안전하다.
// SDK 기본값에만 맡기면 디바이스 IRQ가 최고 우선순위(0)로 남을 수 있어,
// 이후 ISR-to-RTOS 연동이 추가될 때 문제를 만들기 쉽다.
static void IsoSdk_CanConfigureIrqPriority(void)
{
    INT_SYS_SetPriority(CAN0_ORed_IRQn, APP_RTOS_CAN_IRQ_PRIORITY);
    INT_SYS_SetPriority(CAN0_Error_IRQn, APP_RTOS_CAN_IRQ_PRIORITY);
    INT_SYS_SetPriority(CAN0_ORed_0_15_MB_IRQn, APP_RTOS_CAN_IRQ_PRIORITY);
    INT_SYS_SetPriority(CAN0_ORed_16_31_MB_IRQn, APP_RTOS_CAN_IRQ_PRIORITY);
#if FEATURE_CAN_HAS_WAKE_UP_IRQ
    INT_SYS_SetPriority(CAN0_Wake_Up_IRQn, APP_RTOS_CAN_IRQ_PRIORITY);
#endif
}
...
{
    if (FLEXCAN_DRV_Init(instance,
                         &ISOSDK_SDK_CAN_STATE,
                         &ISOSDK_SDK_CAN_INIT_CONFIG) != STATUS_SUCCESS)
    {
        return 0U;
    }

    // scheduler 시작 뒤에도 CAN IRQ가 FreeRTOS 규칙 안에서 동작하도록
    // controller init 직후 우선순위를 한 번 맞춰 둔다.
    IsoSdk_CanConfigureIrqPriority();
    return 1U;
}
...
// mailbox를 여러 개 병행하거나 burst traffic을 더 직접 다루게 되면 buffer 전략을 다시 분리하는 것이 적절하다.
