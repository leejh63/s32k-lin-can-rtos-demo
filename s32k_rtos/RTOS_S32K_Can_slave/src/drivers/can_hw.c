...
// 성공한 TX 완료 누적 횟수를 반환한다.
// 통신이 살아 있는지 빠르게 판단할 때
// error count와 함께 보면 흐름이 훨씬 잘 보인다.
uint32_t CanHw_GetTxOkCount(const CanHw *hw)
{
    return (hw != NULL) ? hw->tx_ok_count : 0U;
}

// 하드웨어 계층에서 기록한 TX 오류 횟수를 반환한다.
uint32_t CanHw_GetTxErrorCount(const CanHw *hw)
{
    return (hw != NULL) ? hw->tx_error_count : 0U;
}

// 성공한 RX 완료 누적 횟수를 반환한다.
uint32_t CanHw_GetRxOkCount(const CanHw *hw)
{
    return (hw != NULL) ? hw->rx_ok_count : 0U;
}

// 하드웨어 계층에서 기록한 RX 오류 횟수를 반환한다.
uint32_t CanHw_GetRxErrorCount(const CanHw *hw)
{
    return (hw != NULL) ? hw->rx_error_count : 0U;
}

// callback queue가 꽉 차 수신을 버린 횟수를 반환한다.
uint32_t CanHw_GetRxDropCount(const CanHw *hw)
{
    return (hw != NULL) ? hw->rx_drop_count : 0U;
}
